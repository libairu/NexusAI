# 腾讯设计 Ardot 新品体验：CodeBuddy + Ardot MCP，AI 驱动从设计稿到代码的全链路实践

## 前言

设计与开发之间的鸿沟，一直是产品团队效率的最大瓶颈之一。设计师在 Figma 中精心绘制的界面，到了开发手中往往需要重新理解、手动提取 Token、逐像素还原 -- 这个过程不仅耗时，还容易产生还原误差。

**Ardot** 是腾讯推出的新一代 AI 原生设计工具，通过 **MCP（Model Context Protocol）** 协议，让 AI Agent 可以直接操作设计画布 -- 生成设计稿、创建组件库、定义变量 Token，真正实现 **AI 驱动的设计与开发一体化**。
...
**自定义 MCP 服务器配置**：如需在其他 AI 终端中使用 Ardot MCP 功能，只需复制以下 JSON 配置，粘贴到对应工具的 MCP 配置文件中，即可一键启动：
...
> 💡 点击 Ardot 面板中的「一键复制」按钮，即可快速获取配置。

### 1.3 开始体验：给 AI 下达设计指令

MCP 配置完成后，在 AI Agent（如 CodeBuddy）中输入设计指令，AI 将通过 Ardot MCP 直接在画布上生成设计稿。

**示例指令**：

> 使用 Ardot MCP 生成一个知识分享平台的 UI 设计，科技风格，深色模式，以"未来已来，拥抱 AI"为主题。要求包含：顶部导航栏、左侧边栏、Hero Banner、知识卡片列表、右侧 AI 助手面板。

AI Agent 接收到指令后，会自动调用 Ardot MCP 的各类 API，在画布上逐步生成完整的设计稿 -- 无需手动绘制一个像素。
...
## 六、Design to Code：Token 一键打通

### 6.1 Token 映射：ardot → Tailwind v4
...
![NexusAI Dark Mode](./dark-mode.png)
...
![NexusAI Light Mode](./light-mode.png)
